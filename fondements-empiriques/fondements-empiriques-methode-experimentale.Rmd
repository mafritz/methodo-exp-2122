...
Dans ces travaux à propos de la causalité, Pearl [@pearl2000; @pearl2016; @pearl2018book] propose d'imaginer une échelle avec trois marche. Chaque marche qui est montée permet de s'approcher davantage à l'explication d'un mécanisme causale.

![L'échelle de causalité avec trois marches. Adapté de @pearl2018book](images/echelle-causalite.png "L'échelle de causalité avec trois marches. Adapté de @pearl2018book")
...
-   Un score s'approchant de -1 témoigne d'une corrélation *négative*, caractérisée par le fait que A et B ont la tendance à varier de manière opposée : une valeur élevé de A est associé à une faible valeur de B (ou vice-versa) ; ou une valeur faible de A est associée à une valeur élevée de B (ou vice-versa).
...
-   Un score s'approchant de 1 témoigne d'une corrélation *positive*, caractérisée par le fait que A et B ont la tendance à varier de manière spéculaire : une valeur élevé de A est associé à une valeur élevé de B (ou vice-versa) et une faible valeur de A est associé à une faible valeur de B (ou vice-versa).
...
ggplot(correlations, aes(x = x, y = y)) +
  ...
  labs(x = "Phénomène B", y = "Phénomène A") +
  facet_wrap(~type)
