...
La question contre-factuel à laquelle ce type d'expérience essaie de répondre peut se synthétiser de la manière suivante : est-ce que **la même** personne quelconque, indépendamment de ses caractéristiques ou connaissances préalable, peut bénéficier davantage au niveau de l'apprentissage si elle peut avoir accès à un enregistrement vidéo ou à des capsules vidéos ? Pour répondre à cette question, une possibilité est celle de comparer la distribution d'une mesure représentative de l'apprentissage qui est une fonction du type de support utilisé (sans vidéo, playback intégral ou capsules vidéos).
...
idealized_experiment <- bind_rows(sans, playback, capsules) |> 
  mutate(
    condition = factor(condition, levels = c("Sans vidéo", "Playback", "Capsules")),
...
En même temps, pour qu'on puisse parler d'un principe, il faudrait accompagner les données d'une explication causale qui illustre les raisons à la base du pattern de données obtenues. À ce stade de l'échelle de causalité, donc, les activités impliquées sont *imaginer*, *penser de manière rétrospective*, et *comprendre*. Les questions qu'on peut se poser à ce stade sont :
